"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const fin_1 = require("./api/fin");
const openfin_renderer_api_1 = require("./environment/openfin-renderer-api");
const transport_1 = require("./transport/transport");
const elipc_1 = require("./transport/elipc");
const openfin_env_1 = require("./environment/openfin-env");
const environment = new openfin_env_1.default();
const transport = new transport_1.default(elipc_1.default, environment, { ...openfin_renderer_api_1.currentWindowIdentity });
transport.connectSync();
window.fin = Object.assign(window.fin, new fin_1.default(transport));
